...
location=$(find /path/to/search/directory -type f -name "*.ovpn" 2>/dev/null)
...
if [ "$distro" = "Debian" ] || [ "$distro" = "Ubuntu" ]; then
    ...
    echo "Your Distro can not be install useding this install script sorry!"
    echo "Distro's that can be install include ( Debian / Ubuntu / Arch )"
    exit 1
fi
...
if [ -z "$location" ];
    ...
    if [ "$answer" = "Y" ]; then
        ...
        echo "Okay what is you .ovpn file called including the .ovpn at the end?"
        read -r ovpn
